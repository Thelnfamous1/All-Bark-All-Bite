...
public class COTWLootTableProvider extends LootTableProvider {
    private final List<Pair<Supplier<Consumer<BiConsumer<ResourceLocation, LootTable.Builder>>>, LootContextParamSet>> subProviders =
            ImmutableList.of(
                    Pair.of(COTWGiftLoot::new, LootContextParamSets.GIFT));

    public COTWLootTableProvider(DataGenerator generator) {
        super(generator);
    }
    ...
    protected void validate(Map<ResourceLocation, LootTable> map, ValidationContext validationContext) {
        for(ResourceLocation resourcelocation : Sets.difference(COTWBuiltInLootTables.all(), map.keySet())) {
