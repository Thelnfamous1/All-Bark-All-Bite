...
public class WolfGoalPackages {
    ...
    static ImmutableList<? extends Pair<Integer, ? extends Behavior<? super Wolf>>> getCorePackage() {
        return BrainUtil.createPriorityPairs(0,
                ImmutableList.of(
                        ...
                        new Retaliate<>(WolfGoalPackages::wasHurtBy),
    ...
    static RunOne<Wolf> createIdleMovementBehaviors() {
        return new RunOne<>(
                ImmutableList.of(
                        ...
                        Pair.of(InteractWith.of(EntityType.WOLF, SharedWolfAi.INTERACTION_RANGE, MemoryModuleType.INTERACTION_TARGET, SharedWolfAi.SPEED_MODIFIER_WALKING, 2), 2),
        ...
        return BrainUtil.createPriorityPairs(0,
                ImmutableList.of(
                        ...
                        new LongJumpToTarget<>(TIME_BETWEEN_LONG_JUMPS, MAX_JUMP_VELOCITY,
                                wolf -> SoundEvents.GOAT_LONG_JUMP))
        );
    }
    ...
    static boolean wantsToStalk(Wolf wolf, LivingEntity target){
        return SharedWolfAi.canStartAttacking(wolf)
    ...
    static ImmutableList<? extends Pair<Integer, ? extends Behavior<? super Wolf>>> getIdlePackage() {
        return BrainUtil.createPriorityPairs(0, ImmutableList.of(
                ...
                new FollowPackLeader<>(SharedWolfAi.ADULT_FOLLOW_RANGE, SharedWolfAi::getMaxPackSize, SharedWolfAi.SPEED_MODIFIER_FOLLOWING_ADULT),
    ...
    private static boolean canAlert(Wolf wolf, LivingEntity other){
        if(wolf.getType() == other.getType()){
            return !AiUtil.hasAnyMemory(other,
                    ...
                    && PackAi.canLead(other, wolf, SharedWolfAi.getMaxPackSize(wolf));
        }
        return false;
    }
    ...
    private static boolean canHunt(Wolf wolf){
        return AiUtil.hasAnyMemory(wolf, MemoryModuleType.ANGRY_AT, COTWMemoryModuleTypes.STALK_TARGET.get())
                && !wolf.isBaby()
