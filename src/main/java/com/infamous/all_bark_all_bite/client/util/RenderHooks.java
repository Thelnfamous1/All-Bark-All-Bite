...
public class RenderHooks {
    ...
    public static void setupWolfRenderRotations(LivingEntity wolf, PoseStack poseStack, float lerpYBodyRot, float partialTick){
        ...
        poseStack.mulPose(Axis.YP.rotationDegrees(180.0F - lerpYBodyRot));
        ...
        if (wolf.deathTime > 0) {
            ...
            poseStack.mulPose(Axis.ZP.rotationDegrees(deathFlipProgress * 90.0F));
        } else if (wolf.isAutoSpinAttack()) {
            poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F - wolf.getXRot()));
            poseStack.mulPose(Axis.YP.rotationDegrees(((float)wolf.tickCount + partialTick) * -75.0F));
